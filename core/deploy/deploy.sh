...
# verifies a release TAG's assets (the two service binaries, optional nsk, deploy tarball, UI), atomically
# activates them, reinstalls units + Caddy edge so the box can't drift, restarts both app services, waits for
