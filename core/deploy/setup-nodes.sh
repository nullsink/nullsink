...
note "3. DRAIN THE RAIL FIRST (money-safety, and the SOLE defense — there is no auto-relabel): on the APP box,"
note "   remove 'bitcoin' from PAY_RAILS in /etc/nullsink.env + systemctl restart nullsink, BEFORE backupwallet."
...
note "   keep the window short, prefer draining when open BTC orders are ~0."
note "4. MIGRATE the watch-only wallet from the app box (backupwallet -> copy over WG -> restorewallet). Do NOT"
note "   re-import from the xpub: an order's PK is the wallet's derivation index — a fresh keypool collides"
note "   with keyed orders, and a pruned node can't rescan."
note "5. systemctl enable --now bitcoind; let it sync (initialblockdownload:false) BEFORE the app box cuts over."
note "6. Provision rpcauth: sudo PRINT_PASSWORD=1 deploy/regen-bitcoin-rpcauth.sh here; paste the printed"
note "   BITCOIN_RPC_PASSWORD into the app box's /etc/nullsink.env; set BITCOIN_RPC_URL=http://<NODE_WG_IP>:8332/wallet/nullsink."
note "7. RE-ADD 'bitcoin' to PAY_RAILS IN ITS ORIGINAL POSITION (the first rail is the /buy default) +"
