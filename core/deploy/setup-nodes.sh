...
note "   rpcallowip=127.0.0.1, rpcallowip=<APP_WG_IP>. Do NOT add wallet=nullsink yet — the wallet arrives in"
note "   step 4, and bitcoind refuses to start when a conf-listed wallet is missing."
note "3. systemctl enable --now bitcoind; let IBD FINISH (initialblockdownload:false) before going further."
note "   This takes hours-to-days and the app box's rail stays fully live throughout — the drain must NOT"
note "   span the sync: a pruned node can't restorewallet a snapshot whose best block has fallen behind its"
note "   prune window, and the drain window should be minutes, not days."
note "4. DRAIN THE RAIL (money-safety, and the SOLE defense — there is no auto-relabel): on the APP box, remove"
note "   'bitcoin' from PAY_RAILS in /etc/nullsink.env + systemctl restart nullsink. Only THEN migrate the"
note "   watch-only wallet: backupwallet on the app box -> copy over WG -> restorewallet here -> add"
note "   wallet=nullsink to the conf -> systemctl restart bitcoind. Do NOT re-import from the xpub (an order's"
note "   PK is the wallet's derivation index — a fresh keypool collides with keyed orders; no rescan on pruned)."
note "   A /buy served after the snapshot derives an address the migrated wallet never recorded -> the deposit"
note "   is invisible (paid-but-uncredited; manual recovery = re-setlabel from pending.db). Open orders freeze"
note "   during the drain (reaping is rail-scoped) but their /order-status renders in the wrong coin's scale —"
note "   keep this window short, prefer draining when open BTC orders are ~0."
note "5. Provision rpcauth: sudo PRINT_PASSWORD=1 deploy/regen-bitcoin-rpcauth.sh here; paste the printed"
note "   BITCOIN_RPC_PASSWORD into the app box's /etc/nullsink.env; set BITCOIN_RPC_URL=http://<NODE_WG_IP>:8332/wallet/nullsink."
note "6. RE-ADD 'bitcoin' to PAY_RAILS IN ITS ORIGINAL POSITION (the first rail is the /buy default) +"
note "   systemctl restart nullsink; then verify instantly: systemctl start status-check.service &&"
note "   journalctl -u status-check.service -n 30 (its BTC probe uses the app's exact Basic-auth path)."
note "7. DECOMMISSION the app box's local bitcoind once step 6 verifies (or it pages forever when it dies):"
note "   on the APP box: systemctl disable --now bitcoind; reclaim /var/lib/bitcoind when comfortable."
