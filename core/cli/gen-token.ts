...
// funds the hash; POST /buy never sees the raw token. The proxy later receives it over TLS for API
// authentication and hashes it in-process. This is stronger than cli/issue.ts, which mints server-side.
//
...
console.log(`  ${token}\n`);
console.log('Hash — send this to POST /buy as "hash" with X-Nullsink-Quote-Contract: 2:\n');
console.log(`  ${hash}\n`);
