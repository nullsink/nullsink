#!/usr/bin/env bash
# Compare the pinned external versions in deploy/lib.sh against upstream's latest release and report any
...
check "tinfoil-proxy" "$tf_pinned" tinfoilsh/tinfoil-proxy ""   # empty prefix: keep the v on both sides
