...
read-only operator tools, and the box deploy machinery. Bun + TypeScript, zero runtime dependencies.
...
cli/      read-only operator CLI + aggregate-report and buyer/dev tools
...
| `bun run build:nsk` | compile the read-only `nsk` CLI |
...
- [cli/README.md](cli/README.md) — read-only operator and local command-line tools
