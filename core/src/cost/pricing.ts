...
// Coerce an upstream-reported count to a sane, finite, non-negative number. A buffered Anthropic body passes
// usage through verbatim and the streaming scanners only `?? 0`, so a parseable-but-mistyped field (a string,
// a negative, a non-finite) from a buggy/hostile upstream would otherwise reach the arithmetic below and bill
// NaN (corrupting the ledger) or a negative (minting balance). Floor every term at a clean 0 here — the one
// place all providers and both the buffered and streaming paths funnel through. Valid counts are unchanged.
...
export function costOf(rate: Rate, usage: Usage): number {
  ...
  const cacheWrite = num(usage.cache_creation_input_tokens); // total: 5-min + 1-hour
  ...
  const write1h = Math.min(num(usage.cache_creation_1h_input_tokens), cacheWrite);
