...
import { parseFinancialReport, renderFinancialReport } from "../cli/report-financials";
...
const FINANCIALS_CLI = fileURLToPath(new URL("../cli/report-financials.ts", import.meta.url));
...
test("renders exact aggregate financials from a finalized report", () => {
  ...
  expect(output).toContain("open orders: 0 · credit: $0.000000 · payment seen: 0");
  expect(output).toContain("undelivered credits: 0 · amount: $0.000000 · oldest: none");
  expect(output).toContain("source: backup-20260724T080000Z.tar.age · restore-dry-run-ok");
});

test("renders open-order and undelivered-credit diagnostics", () => {
  const value = fixture();
  value.operations = {
    open_orders: { count: 2, credit_micros: "12000000", payment_seen: 1 },
    undelivered_credits: { count: 1, micros: "8000000", oldest_age_seconds: 42 },
  };
  const output = renderFinancialReport(parseFinancialReport(JSON.stringify(value)));
  expect(output).toContain("open orders: 2 · credit: $12.000000 · payment seen: 1");
  expect(output).toContain("undelivered credits: 1 · amount: $8.000000 · oldest: 42s");
});
