...
test("non-billable outcomes refund in full (status + zero net debit)", async () => {
  ...
  await fc.assert(
    fc.asyncProperty(tokenArb, modelArb, fc.integer({ min: 1, max: 200_000 }), outcomeArb, async (token, model, maxTokens, outcome) => {
      ...
      switch (outcome.kind) {
        case "non2xx":
          // The provider's exact 5xx is transparent; it is still non-billable and refunded in full.
...
// Non-OK responses are always refunded. Every upstream response relays verbatim except errors describing
// nullsink's own upstream credentials/account/billing state, which stay opaque and terminal.
test("upstream errors: exact details relay while operator auth/billing stays private (always refunded)", async () => {
  const errSpy = spyOn(console, "error").mockImplementation(() => {});
  const model = "claude-opus-4-8";
  const cases = [
    { up: 400, body: JSON.stringify({ type: "error", error: { type: "invalid_request_error", message: "max_tokens: too large for model" } }), want: 400, relayed: true, retry: "false", label: "generic 400 invalid_request" },
    ...
    { up: 400, body: JSON.stringify({ type: "error", error: { type: "invalid_request_error", message: "messages.0.content: too long; trim your notes about billing and quota usage" } }), want: 400, relayed: true, retry: "false", label: "400 echoing billing/quota in user text (must relay)" },
    { up: 400, body: JSON.stringify({ error: null, note: "Your credit balance is too low" }), want: 400, relayed: true, retry: "false", label: "billing phrase outside error object (must relay)" },
    { up: 400, body: JSON.stringify({ error: "Your credit balance is too low" }), want: 400, relayed: true, retry: "false", label: "billing phrase in scalar error (must relay)" },
    { up: 400, body: JSON.stringify({ note: "purchase credits" }), want: 400, relayed: true, retry: "false", label: "billing phrase in sibling JSON field (must relay)" },
    { up: 400, body: JSON.stringify({ error: { type: ["billing_error"], code: ["insufficient_quota"], message: ["purchase credits"] } }), want: 400, relayed: true, retry: "false", label: "coercible non-string billing fields (must relay)" },
    { up: 413, body: JSON.stringify({ type: "error", error: { type: "request_too_large" } }), want: 413, relayed: true, retry: "false", label: "413 too large" },
    ...
    { up: 400, body: JSON.stringify({ error: { type: "billing_error", message: "payment required" } }), want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "structured billing type without phrase" },
    { up: 400, body: JSON.stringify({ error: { code: "insufficient_quota", message: "limit reached" } }), want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "structured billing code without phrase" },
    { up: 400, body: "Your credit balance is too low", want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "non-JSON billing phrase fallback" },
    // OpenAI ships out-of-funds as a 429 insufficient_quota (per its docs), which retrying never clears, so
    // it must mask to 503. A real throttle 429 (below) relays with its actionable provider detail.
    ...
    { up: 407, body: "proxy auth required", want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "407 gateway auth" },
    { up: 500, body: JSON.stringify({ error: { type: "authentication_error", message: "gateway mislabeled auth" } }), want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "structured auth type on non-auth status" },
    { up: 500, body: JSON.stringify({ error: { type: "permission_error", message: "gateway mislabeled permission" } }), want: 503, relayed: false, type: "api_error", message: "service_unavailable", retry: "false", label: "structured permission type on non-auth status" },
    { up: 400, body: JSON.stringify({ error: { code: ["invalid_api_key"], message: "not a string code" } }), want: 400, relayed: true, retry: "false", label: "coercible non-string auth code (must relay)" },
    { up: 429, body: "slow down", want: 429, relayed: true, retry: "true", label: "429 provider throttle" },
    { up: 500, body: JSON.stringify({ type: "error", error: { type: "InternalServerError", message: "Unsupported thinking_effort='minimal'; supported values are ['high', 'low', 'max']." } }), want: 500, relayed: true, retry: "true", label: "misclassified request-validation 500" },
    { up: 529, body: JSON.stringify({ type: "error", error: { type: "overloaded_error", message: "provider overloaded" }, request_id: "req_support" }), want: 529, relayed: true, retry: "true", retryAfter: "12", label: "529 overloaded" },
  ];
  for (const c of cases) {
    ...
    if (c.relayed) {
      ...
      expect([c.label, lower.includes("credit balance") || lower.includes("anthropic") || lower.includes("console") || lower.includes("invalid api key") || lower.includes("forbidden")]).toEqual([c.label, false]);
...
test("relayed transient errors preserve Retry-After and the exact provider body", async () => {
  const errSpy = spyOn(console, "error").mockImplementation(() => {});
...
test("streaming: a non-2xx upstream relays exact detail and refunds in full", async () => {
  ...
  expect(j.error.message).toBe("LEAK-MARKER-org-acme-42 slow down");
  expect(balances.getBalance(hashToken(token))).toBe(initial);
});
