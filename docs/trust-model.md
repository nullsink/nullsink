...
| **Payment linkage stays out of the balance ledger** | Payment-side state lives in `pending.db`, separate from balances. `balances.db` keeps no payment-to-token mapping. This is a storage boundary, not erasure: acknowledged `credit_outbox` rows currently retain the token hash, amount, and transaction-derived idempotency key. See [invariants.md](invariants.md#why-are-acknowledged-outbox-rows-kept). |
...
- **On-chain deposits may be public.** The privacy of a deposit depends on the coin: Monero shields
  amounts and addresses, while Bitcoin is transparent. The payments database can associate a deposit
  with a token hash, including after settlement through the retained credit outbox. The separation from
  `balances.db` limits where that association lives; it does not erase it. The watch-only wallets also
  retain the addresses they generated.
