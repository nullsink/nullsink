...
| **Errors don't leak our key or billing state** | `handler.ts` (`relayOrSanitizeUpstream`) relays upstream errors verbatim—including exact 5xx validation and outage detail—but keeps only operator key, permission, and billing failures opaque. |
