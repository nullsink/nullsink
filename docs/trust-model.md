...
| **Your key and identity never leak upstream** | `http/headers.ts` strips the headers that identify the caller or our account before forwarding — the client's auth (we inject our own), cookies, any org/beta headers, and the client's SDK fingerprint — and scrubs cookies plus our org/project headers off responses; the exact list is the `STRIP` set in the source. Caddy removes `Set-Cookie` again at the public edge. |
