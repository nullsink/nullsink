...
| **No request logs, no content retention** | `log.ts` records only operational lines — never a per-request entry, and never a user-linkable pair (no token hash beside a txid or address). Prompts and outputs stream straight through and are never stored; `providers/openai.ts` also forces `store:false`, so the upstream keeps nothing either. Aggregate metrics are kept — see *What we do collect*, below. |
